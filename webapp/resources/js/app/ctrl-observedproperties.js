angular.module('gossamerApp', []).controller('EntityController', ['$scope','$http',
    function EntityController($scope, $http) {
        $scope.data = {};
        $http.get('v1.0/ObservedProperties').success(function(data) {
            ...
            $scope.data.iconColor = "entity-box-col-boxochocs";

            $scope.entityClick = function(id) {
                $http.get('v1.0/ObservedProperties(' + id + ')').success(function(data) {
                    $scope.data.entity = {
                        id: data["@iot.id"],
                        text: data.name
                    }
